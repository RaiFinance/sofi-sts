...
contract Portfolio is ERC20 {
    ...
    mapping(address => IPortfolio.ModuleState) public moduleStates;

    bool public isLocked;

    address[] public components;

    mapping(address => IPortfolio.ComponentPosition) private componentPositions;
        ...
        ERC20(_name, _symbol)
    {
        ...
        for (uint256 i = 0; i < _modules.length; i++) {
            moduleStates[_modules[i]] = IPortfolio.ModuleState.PENDING;
        }
    ...
    function addModule(address _module) external onlyManager {
        require(moduleStates[_module] == IPortfolio.ModuleState.NONE, "Module must not be added");
        require(controller.isModule(_module), "Must be enabled on Controller");

        moduleStates[_module] = IPortfolio.ModuleState.PENDING;
    ...
    function removeModule(address _module) external onlyManager {
        ...
        require(moduleStates[_module] == IPortfolio.ModuleState.INITIALIZED, "Module must be added");

        IModule(_module).removeModule();

        moduleStates[_module] = IPortfolio.ModuleState.NONE;
    ...
    function removePendingModule(address _module) external onlyManager {
        require(!isLocked, "Only when unlocked");
        require(moduleStates[_module] == IPortfolio.ModuleState.PENDING, "Module must be pending");
    ...
    function initializeModule() external {
        require(!isLocked, "Only when unlocked");
        require(moduleStates[msg.sender] == IPortfolio.ModuleState.PENDING, "Module must be pending");
        
        moduleStates[msg.sender] = IPortfolio.ModuleState.INITIALIZED;
    ...
    function portfolioManager(address _manager) external onlyManager {
        require(!isLocked, "Only when unlocked");
    ...
    function isInitializedModule(address _module) external view returns (bool) {
        return moduleStates[_module] == IPortfolio.ModuleState.INITIALIZED;
    }

    
    function isPendingModule(address _module) external view returns (bool) {
        return moduleStates[_module] == IPortfolio.ModuleState.PENDING;
    }

    
    function getPositions() external view returns (IPortfolio.Position[] memory) {
        IPortfolio.Position[] memory positions = new IPortfolio.Position[](_getPositionCount());
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            if (_defaultPositionVirtualUnit(component) > 0) {
                positions[positionCount] = IPortfolio.Position({
                    component: component,
                    module: address(0),
